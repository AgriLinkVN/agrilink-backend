...
## P8-06 TEST Persistence Inventory Overlay

The 15 current database-capable TEST fixture/harness sources, their 30
business fixture tables, raw SQL classifications, owner matrix, guard gaps,
and proposed 15-group dependency design are authoritative in
...
## P8-10 Final Phase 8 Closure Overlay

The final database-free inventory re-derives 11 executable canonical groups
from source: three REFERENCE, five DEV, and three TEST. Their 14 written tables
have one bounded-context owner each, all seven dependency edges are declared
and acyclic, and no cross-owner seed repository/entity access exists. Merged
PR #154 supplies runtime idempotency authority. See the
[final closure](final-closure.md).
...
## P8-06C Clean-v2 Owner Provider Split Inventory Overlay

P8-06C adds one owner-local TEST group and moves one of the fifteen clean-v2
raw SQL prerequisite declarations. Detailed disposition evidence remains in
[test-fixture-ownership-audit.md](test-fixture-ownership-audit.md).

| TEST group | Owner | Stable key | Tables | Dependencies | Output | Source |
| --- | --- | --- | --- | --- | --- | --- |
| `admin.test.system-configs` | Admin | `key` | `system_configs` | none | `system-config.id.by-key` | TF-02 `CV2-F13` |
...
## P8-06B0 Product TEST Identity Inventory Overlay

The B0 audit preserves all runtime inventory. TF-02, TF-04, and TF-05 each
persist one ordinary Product without SKU; TF-08 keeps four SKU-less minimal
Product parents inside its legacy Storage migration scenario. Details and the
source/history evidence are maintained in
[test-fixture-ownership-audit.md](test-fixture-ownership-audit.md).
...
TF02_PRODUCT_COUNT=1
TF02_PRODUCT_SKU_EXPLICIT=NO
TF04_PERSISTED_PRODUCT_COUNT=1
TF04_PRODUCT_SKU_EXPLICIT=NO
TF05_PRODUCT_COUNT=1
TF05_PRODUCT_SKU_EXPLICIT=NO
TF08_MIGRATION_LOCAL_PRODUCT_COUNT=4
TF08_PRODUCT_SKU_REPRESENTED=NO
...
## P8-06A TEST Safety Inventory Overlay

The historical 15-source inventory remains unchanged. The post-A guard and
purpose matrix is maintained in
[test-fixture-ownership-audit.md](test-fixture-ownership-audit.md).
...
## P8-06B0 Trailing Current-Authority Inventory Handoff
...
## P8-06B Revised Shared TEST Provider Inventory Overlay

The B0 authority is merged by PR #146. P8-06B now adds only the reusable
TF-04/TF-05 seller and Commerce Rice Product. The clean-v2 Phase One Product
and TF-08 migration Products remain local to their existing harnesses. See
[test-fixture-ownership-audit.md](test-fixture-ownership-audit.md) for payload,
identity, boundary, and stopped-attempt evidence.

| Executable TEST group | Owner | Records | Stable key | Dependencies | Output |
| --- | --- | ---: | --- | --- | --- |
| `users.test.identities` | Users | 1 | email | none | `user.id.by-email` |
| `products.test.catalog` | Products | 1 | SKU `TEST-COMMERCE-RICE-001` | `users.test.identities` | `product.id.by-sku` |
...
NEW_BUSINESS_TEST_SEED_GROUP_COUNT=2
NEW_OTHER_BUSINESS_TEST_GROUP_COUNT=0
...
## P8-06C Trailing Current-Authority Inventory Handoff
...
## P8-06D Commerce Harness Shared Provider Consumption Inventory Overlay

P8-06D creates no TEST group and changes no provider payload. TF-04 and TF-05
now use a TEST-only output adapter over the existing shared identity registry.
Each harness removes one direct shared seller declaration and one direct Rice
Product declaration while retaining its unrelated User fixtures.

| Harness | Shared provider | Output kind | Stable key | Direct reusable rows before | Direct reusable rows after |
| --- | --- | --- | --- | ---: | ---: |
| TF-04 | `users.test.identities` | `user.id.by-email` | `seller@example.test` | 1 | 0 |
| TF-04 | `products.test.catalog` | `product.id.by-sku` | `TEST-COMMERCE-RICE-001` | 1 | 0 |
| TF-05 | `users.test.identities` | `user.id.by-email` | `seller@example.test` | 1 | 0 |
| TF-05 | `products.test.catalog` | `product.id.by-sku` | `TEST-COMMERCE-RICE-001` | 1 | 0 |
...
USERS_TEST_RECORD_COUNT=1
PRODUCTS_TEST_RECORD_COUNT=1
P8_06D_EXISTING_TEST_GROUP_COUNT=3
...
## P8-06D1 DataSource Target Binding Inventory Overlay

The P8-06D TEST-only adapter remains payload-free and continues to execute the
same two registered groups. Before constructing those repository-backed
groups, it now validates both the requested target and the supplied
`DataSource` target with the existing P8-06A guard and requires exact database
and normalized-host equality. Split PostgreSQL options and PostgreSQL URL
options are supported; incomplete or ambiguous targets are rejected.
...
## P8-06E TEST Harness Consistency And Closure Inventory Overlay

The final P8-06 inventory contains exactly three owner-local TEST groups. The
only dependency edge is Products consuming the Users email-backed scalar
output. No TEST group depends on DEV, and all remaining TEST persistence writes
are classified as bounded workflow, synthetic compatibility, migration-local,
catalog, or read-only harness behavior.

| Group | Owner | Stable key | Dependencies | Output |
| --- | --- | --- | --- | --- |
| `users.test.identities` | Users | email | none | `user.id.by-email` |
| `products.test.catalog` | Products | SKU | `users.test.identities` | `product.id.by-sku` |
| `admin.test.system-configs` | Admin | config key | none | `system-config.id.by-key` |
...
## P8-08 Idempotency And Retry Readiness Overlay

PR #151 was human-reviewed, passed the Backend Quality Gate, and merged into
`develop` at `eb25bddebf35be98c9f1a59448016c47fcef47e3`. The current 11-group
inventory was audited per record for duplicate prevention, state convergence,
persisted-ID output stability, and partial-run retry. Seven owner-local groups
received bounded mutable-payload or create-only corrections. See the
[P8-08 readiness decision record](seed-idempotency-readiness.md) for the full
identity matrix and nondeterministic-field decisions.
...
The canonical inventory contains 14 unique written tables. The empty-database
V2 head supplied 13; `cooperative_members` was the only missing table. The
current entity, writer identity, domain status union, and reviewed legacy P3
constraints support one forward-only migration without seed changes. Full
