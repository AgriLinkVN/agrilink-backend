...
PR #150 merged the P8-06E closure at
`b50596d9c410c43882e830c8a4c0a19cfad91951`. P8-07 combines its three
TEST groups with the current three REFERENCE and five DEV groups for one
database-free metadata audit. The resulting 11-node, seven-edge graph is
complete and acyclic. Its only TEST edge remains
`products.test.catalog -> users.test.identities`; no TEST group depends on DEV
and no normal startup or CLI path imports the TEST registry.

All seven dependencies use a declared scalar output contract. Six cross-owner
edges cross only application contracts: Cooperative Members, Products,
Profiles, and Reviews consume Users output, Reviews consumes Products output,
and TEST Products consumes TEST Users output. The remaining Products DEV to
Product Categories edge stays within the Products owner. No cross-owner
repository or entity access was found.
...
`develop` at `eb25bddebf35be98c9f1a59448016c47fcef47e3`. P8-08 preserves the
three-group TEST registry and its actual-DataSource target binding. No TEST
provider was added. The TEST Users, Products, and Admin groups reconcile by
email, SKU, and config key respectively, reject ambiguous array results, and
publish actual persisted IDs. See the
