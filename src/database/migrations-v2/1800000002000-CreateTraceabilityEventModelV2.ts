...
/** Additive Phase 7B schema; execution requires an approved disposable target. */
export class CreateTraceabilityEventModelV21800000002000 implements MigrationInterface {
  ...
  async up(queryRunner: QueryRunner): Promise<void> {
    await queryRunner.query(`CREATE TABLE "traceability_batches" (
      ...
      "batch_code" varchar(100) NOT NULL, "qr_code" varchar(100) NOT NULL,
      "created_at" timestamptz NOT NULL DEFAULT now(),
      CONSTRAINT "UQ_traceability_batches_batch_code" UNIQUE ("batch_code"),
      CONSTRAINT "UQ_traceability_batches_qr_code" UNIQUE ("qr_code")
    )`);
    await queryRunner.query(`CREATE TABLE "traceability_events" (
      ...
      "evidence_file_ids" uuid[] NOT NULL DEFAULT '{}', "operation_key" varchar(128) NOT NULL,
      "supersedes_event_id" uuid, "created_at" timestamptz NOT NULL DEFAULT now(),
      CONSTRAINT "UQ_traceability_events_operation_key" UNIQUE ("operation_key"),
      CONSTRAINT "UQ_traceability_events_batch_sequence" UNIQUE ("batch_id", "sequence"),
      CONSTRAINT "CHK_traceability_events_event_kind" CHECK ("event_kind" IN ('BATCH_CREATED','PLANTED','HARVESTED','QUALITY_TESTED','SHIPPED','CORRECTED')),
    ...
    await queryRunner.query(
      'CREATE INDEX "IDX_traceability_events_batch_sequence" ON "traceability_events" ("batch_id", "sequence")',
    );
  }

  async down(): Promise<void> {
    throw new Error(
      "Traceability event evidence is append-only; rollback requires an approved staged reconciliation plan.",
    );
  }
}
