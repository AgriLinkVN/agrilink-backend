...
} from '../../application/ports/outbound/stored-file-repository.port';
...
export class TypeOrmStoredFileRepository implements StoredFileRepositoryPort {
  ...
  async create(file: StoredFileModel): Promise<StoredFileModel> {
    return this.toModel(
      await this.files.save(
        this.files.create({ ...file, sizeBytes: String(file.sizeBytes) }),
      ),
    );
  }
  async findById(id: string): Promise<StoredFileModel | null> {
    const entity = await this.files.findOne({ where: { id } });
    return entity ? this.toModel(entity) : null;
  }
  async findExpiredPending(now: Date): Promise<StoredFileModel[]> {
    return (
      await this.files.find({
        where: { status: 'PENDING', expiresAt: LessThan(now) },
      })
    ).map((file) => this.toModel(file));
  }
  async findDeletionRetries(): Promise<StoredFileModel[]> {
    return (await this.files.find({ where: { status: 'DELETE_RETRY' } })).map(
  ...
  async findByIdForOwner(
    ...
    const entity = await this.files.findOne({ where: { id, ownerId } });
    return entity ? this.toModel(entity) : null;
  }
  async updateStatus(
    id: string,
    ...
    await this.files.update({ id, ownerId }, { status, ...metadata });
  ...
  async attachToResource(
    ...
    const result = await this.files
      ...
      .andWhere('"assetType" = :assetType', { assetType })
      .andWhere("provider = 'SUPABASE'")
      .andWhere("visibility = 'PRIVATE'")
      .andWhere("status IN ('QUARANTINED', 'ACTIVE')")
      .andWhere(
        '(resource_id IS NULL OR (resource_type = :resourceType AND resource_id = :resourceId))',
  ...
  private toModel(entity: StoredFileEntity): StoredFileModel {
    return {
      ...
      declaredMime: entity.declaredMime,
      detectedMime: entity.detectedMime,
      checksumSha256: entity.checksumSha256,
      extension: entity.extension,
      deletionAttempts: entity.deletionAttempts,
