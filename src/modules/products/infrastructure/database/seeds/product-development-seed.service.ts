...
@Injectable()
export class ProductDevelopmentSeedService {
  constructor(
  ...
  async seedForDevelopment(options: { reset?: boolean } = {}): Promise<{
    deleted: number;
    seeded: number;
    skipped: number;
  }> {
    ...
    const result = await this.seedMockData();
    return { deleted: 0, ...result };
  }

  private async seedMockData(): Promise<{ seeded: number; skipped: number }> {
