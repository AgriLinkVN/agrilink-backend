...
describe("Admin state-agency oversight REST contract (e2e)", () => {
  ...
  describe("profile verification", () => {
    it("allows a state agency reviewer and preserves the verify contract", async () => {
      adminService.verifyProfile.mockResolvedValue({
        success: true,
        profile: { id: FARMER_ID, isKycVerified: false },
      });

      await request(server)
        .patch(`/admin/profiles/farmer/${FARMER_ID}/verify`)
        .set("Authorization", "Bearer token")
        .set("x-user-id", STATE_AGENCY_ID)
        .set("x-user-role", UserRole.STATE_AGENCY)
        .send({ isApproved: false, rejectionReason: "Document mismatch" })
        .expect(200);

      expect(adminService.verifyProfile).toHaveBeenCalledWith(
        "farmer",
        FARMER_ID,
    ...
    it("rejects a normal user and a rejection without a reason", async () => {
      await request(server)
        .patch(`/admin/profiles/farmer/${FARMER_ID}/verify`)
        .set("Authorization", "Bearer token")
        .set("x-user-id", FARMER_ID)
  ...
  describe("GET /admin/products/violating", () => {
    it("returns suspended/rejected products with the seller attached", async () => {
      ...
      expect(res.body.data.data[0].seller).toEqual({
  ...
  describe("GET /admin/cooperatives-enterprises", () => {
    it("returns both cooperative and enterprise lists", async () => {
      adminService.getCooperativesAndEnterprises.mockResolvedValue({
        cooperatives: [
          { id: "c1", name: "HTX Rau sach Da Lat", taxCode: "0312345678" },
        ],
        enterprises: [
          { id: "e1", name: "Cong ty XNK Nong san", taxCode: "0398765432" },
        ],
      });
  ...
  describe("GET /admin/reports/system.pdf", () => {
    it("streams a PDF with the correct content type", async () => {
      adminReportService.generateSystemReportPdf.mockResolvedValue(
        Buffer.from("%PDF-1.3 fake"),
      );
    ...
    it("propagates a report-generation failure as 500 without crashing the process", async () => {
      adminReportService.generateSystemReportPdf.mockRejectedValueOnce(
        new Error("pdfkit blew up"),
      );
...
function testUserMiddleware(
  ...
  if (req.headers.authorization) {
    req.user = {
      ...
      role:
        (headerValue(req, "x-user-role") as UserRole) ?? UserRole.STATE_AGENCY,
    };
  }
  next();
}
